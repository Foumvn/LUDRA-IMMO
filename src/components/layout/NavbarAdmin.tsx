// components/layout/NavbarAdmin.tsx
'use client';

import { memo, useState, useRef, useEffect } from 'react';
import { useTranslation } from 'react-i18next';
import { Menu, X, Home, Heart, Users, BarChart3, Settings, LogOut, User } from 'lucide-react';
...
const NavbarAdmin = memo(() => {
  ...
  const navigation = [
    { name: 'Tableau de bord', href: URL.admin.dashboard, icon: BarChart3 },
    { name: 'Utilisateurs', href: URL.admin.users, icon: Users },
    { name: 'Propriétés', href: URL.admin.properties, icon: Home },
    // { name: 'Rapports', href: URL.admin.reports, icon: BarChart3 },
  ];
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-secondary backdrop-blur-md border-b border-gray-200">
        ...
        <div className="flex justify-between items-center h-16">
          ...
          <div className="flex-shrink-0 flex items-center">
            <Link href={URL.public.home} className="text-2xl font-bold text-white hover:opacity-80 transition-opacity">
              House<span className="text-primary">Plateforme</span> 
            </Link>
          </div>
          ...
          <div className="hidden md:flex items-center space-x-4">
            <div className="relative" ref={profileMenuRef}>
              <button
                ...
                className="flex items-center space-x-2 px-3 py-2 rounded-lg border border-gray-200 hover:border-secondary hover:bg-secondary-300 transition-colors"
              >
              ...
              {navigation.map((item) => {
                ...
                return (
                  <Link
                    key={item.name}
                    href={item.href}
                    className="flex items-center text-primary hover:text-primary hover:bg-gray-50 px-3 py-3 rounded-md text-base font-medium transition-colors duration-300"
                    onClick={closeMenu}
                  >
